        ...
        {result1.map((e,i)=>{
          return(
              <tr  key={i}>
                ...
                <td>  
					<DropdownButton className="text-center" id="dropdown-item-button" title="Action" variant="secondary" onClick={()=> setArchiveStatus(e._id)}>
					  <Dropdown.ItemText>TAKE ACTION</Dropdown.ItemText>					  
					  <Dropdown.Item as="button" onClick={()=> {archiveState === 'unarchive'?archiveAdmin(e._id):unArchiveAdmin(e._id)} }>{archiveState === 'unarchive'?'SUSPEND':'ACTIVE'}</Dropdown.Item>
