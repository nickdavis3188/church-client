...
export default function BulkUpdate() {
  ...
  const getMemsForUpdate = async()=>{
	   ...
	   let userId2 = JSON.stringify({date:`${myDate}`})
        ...
        if(data.status === 'success'){
			...
			let result = _.map(uuu, function(currentObject) {
				return _.pick(currentObject,"RegNumber","Firstname","Surname","Email","PhoneNo","MaritalStatus","Address","Occupation","Business","_id");
			});
	...
	const submitData = async(e)=>{
		...
		let result33 = _.map(fillData, function(currentObject2) {
			return _.pick(currentObject2,"RegNumber","Firstname","Surname","Email","PhoneNo","MaritalStatus","Address","Occupation","Business","_id");
		});
  ...
  return (
    ...
    />
	<br/>
	<button className="btn btn-primary px-4" onClick={(e)=> submitData(e)} ><FaFileUpload/>Update</button>
	<ToastContainer/>
	<br/>
	<br/>
	</>
