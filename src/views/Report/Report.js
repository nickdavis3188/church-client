		...
		if(myDate == '' && selectJourney == 0){
			...
			if(typeof(selectJourney) == 'number'){
				...
				if(data){
					...
					let aarr = []
					data.data.map((ee,i)=>aarr.push({RegNumber:ee.MemberId.RegNumber,Firstname:ee.MemberId.Firstname,Surname:ee.MemberId.Surname,Email:ee.MemberId.Email,PhoneNo:ee.MemberId.PhoneNo,JourneyName:ee.JourneyId.JourneyName,Status:ee.Status,AttendDate:formatDate2(ee.JourneyDate),Initiator:ee.AdminId.fullName,InitiatorRole:ee.AdminId.role}))
		...
		<div>
			<div className="input-group" >
				<div className='form-outline'>					
					<DropdownButton className="text-center" id="dropdown-item-button" title="Journey" variant="secondary">
					  <Dropdown.ItemText>TAKE ACTION</Dropdown.ItemText>
					  <Dropdown.Item as="button" onClick={()=>setSelectJourney(1)} >Journey 101</Dropdown.Item>	
					  <Dropdown.Item as="button" onClick={()=>setSelectJourney(2)}>Journey 201</Dropdown.Item>	
					  <Dropdown.Item as="button" onClick={()=>setSelectJourney(3)}>Journey 202</Dropdown.Item>
					  <Dropdown.Item as="button" onClick={()=>setSelectJourney(4)}>Journey 301</Dropdown.Item>
					  <Dropdown.Item as="button" onClick={()=>setSelectJourney(5)}>Journey 401</Dropdown.Item>
					</DropdownButton>				
				</div>
				<br/>
				<div className="form-outline">			
					<input id="search-focus" type="date" id="form1" className="form-control" onChange={(e)=> setMyDate(e.target.value)}/>		
				</div>
